...
const updateVehicleIntoDB = async (payload, vehicleId) => {
    const { vehicle_name, type, registration_number, daily_rent_price, availability_status } = payload;
...
const deleteVehicleFromDB = async (vehicleId) => {
    ...
    const vehicleCheck = await pool.query(`SELECT id, vehicle_name FROM vehicles WHERE id = $1`, [vehicleId]);
    if (vehicleCheck.rows.length === 0) {
        throw new Error("Vehicle not found");
    }
    // Step 2: Check if vehicle has any active bookings
    const activeBookings = await pool.query(`SELECT id FROM bookings 
         WHERE vehicle_id = $1 
         AND status = 'active'`, [vehicleId]);
