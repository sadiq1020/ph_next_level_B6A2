...
const bookingIntoDB = async (payload) => {
    ...
    try {
        ...
        const vehicleResult = await client.query(`SELECT id, vehicle_name, daily_rent_price, availability_status 
             FROM vehicles 
...
const getBookingFromDB = async (authenticatedUser) => {
    ...
    if (authenticatedUser.role === 'customer') {
        const result = await pool.query(`SELECT 
             ...
             ORDER BY b.created_at DESC`, [authenticatedUser.id]);
        return result;
    }
    throw new Error("Unauthorized access");
};
//  update booking business logics
const updateBookingIntoDB = async (payload, bookingId, authenticatedUser) => {
    ...
    try {
        ...
        const bookingResult = await client.query(`SELECT bookings.*, vehicles.availability_status as vehicle_status
             FROM bookings
             JOIN vehicles ON bookings.vehicle_id = vehicles.id
             WHERE bookings.id = $1`, [bookingId]); //  here the first line means -> Give me all the columns from the bookings table + just the availability_status (that named here vehicle_status) from the vehicles table
        ...
        if (authenticatedUser.role === 'customer') {
            ...
            const updatedBooking = await client.query(`UPDATE bookings 
                 SET status = $1 
                 WHERE id = $2 
                 RETURNING *`, ['cancelled', bookingId]);
            // Update vehicle status back to available
            await client.query(`UPDATE vehicles 
                 SET availability_status = $1
                 WHERE id = $2`, ['available', booking.vehicle_id]);
            await client.query('COMMIT');
        ...
        if (authenticatedUser.role === 'admin') {
            ...
            const updatedBooking = await client.query(`UPDATE bookings 
                 SET status = $1
                 WHERE id = $2 
                 RETURNING *`, ['returned', bookingId]);
            // Update vehicle status back to available
            const updatedVehicle = await client.query(`UPDATE vehicles 
                 SET availability_status = $1
                 WHERE id = $2
                 RETURNING availability_status`, ['available', booking.vehicle_id]);
            await client.query('COMMIT');
